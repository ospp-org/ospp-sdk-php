...
# the spec registry table in spec/05-state-machines.md, at the ref pinned in .spec-ref.
#
...
if [[ -n "${SPEC_REPO:-}" ]]; then
  SPEC_ROOT="${SPEC_REPO}"
  if [[ ! -f "${SPEC_ROOT}/spec/05-state-machines.md" ]]; then
    echo "ERROR: SPEC_REPO=${SPEC_REPO} but ${SPEC_ROOT}/spec/05-state-machines.md not found" >&2
    exit 1
  fi
